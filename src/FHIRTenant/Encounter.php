...
use Satusehat\Integration\FHIRTenant\Exception\FHIRMissingProperty;
...
class Encounter extends OAuth2ClientTenant
{
    ...
    {
        $identifier['system'] = 'http://sys-ids.kemkes.go.id/encounter/' . $this->profile->organization_id;
    ...
    {
        $this->encounter['serviceProvider']['reference'] = 'Organization/' . $this->profile->organization_id;
    }
    ...
    public function json()
    {
        ...
        if (!array_key_exists('status', $this->encounter)) {
            throw new FHIRMissingProperty('Please use encounter->statusHistory([timestamp array]) to add the status');
        }

        // Class is required
        if (!array_key_exists('class', $this->encounter)) {
            throw new FHIRMissingProperty('Please use encounter->setConsultationMethod($method) to pass the data');
        }

        // Subject is required
        if (!array_key_exists('subject', $this->encounter)) {
            throw new FHIRMissingProperty('Please use encounter->setSubject($subjectId, $name) to pass the data');
        }

        // Participant is required
        if (!array_key_exists('participant', $this->encounter)) {
            throw new FHIRMissingProperty('Please use encounter->addParticipant($participantId, $name) to pass the data');
        }

        // Location is required
        if (!array_key_exists('location', $this->encounter)) {
            throw new FHIRMissingProperty('Please use encounter->addLocation($locationId, $name) to pass the data');
        }
